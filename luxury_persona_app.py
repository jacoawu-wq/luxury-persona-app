...
def get_gemini_analysis(api_key, product, archetype, phase, base_data):
    ...
    try:
        ...
        - **人物畫像與產業背景**：(請深度分析：他們的職業頭銜、所屬產業聚落、財富積累模式。例如：竹科高階主管、傳產中小企業主...)
        - **居住動機與鄰里互動**：(分析動機：為何看上此案？是為了學區還是資產配置？以及社交話題：他們在交誼廳會聊國際局勢還是高爾夫？)
        - **Meta (FB/IG) 廣告建議**：
          - 興趣標籤：(列出 5-8 個精準興趣)
          - 行為/人口統計：(例如：經常出國、企業主...)
        - **Google 搜尋關鍵字建議**：(列出 8-10 個高搜尋意圖的關鍵字)
        - **Google 多媒體 (GDN/YouTube) 受眾建議**：
          - (列出 5 個具體的興趣受眾、頻道或網站類別。例如：Bloomberg 讀者、高爾夫球愛好者、特定的 YouTube 財經頻道、造訪過競爭對手網站的人...)
